...
function UserList() {
  ...
  useEffect(() => {
    const fetchUsersWithPhotoCount = async () => {
      try {
        const response = await axios.get("https://5gz5y6-5000.csb.app/api/user");
        const userList = response.data;

        // Lấy số lượng ảnh cho mỗi người dùng
        const promises = userList.map(async (user) => {
          const res = await axios.get(
            `https://5gz5y6-5000.csb.app/api/photo/getCountPT/${user._id}`,
          );
      ...
      const fetchPhoto = async () => {
        try {
          const response = await axios.get(
            `https://5gz5y6-5000.csb.app/api/photo/${user._id}`,
          );
